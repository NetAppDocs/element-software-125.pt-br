...
NOTE: Os sistemas operacionais diferem na forma como tratam volumes clonados. O VMware ESXi tratará um volume clonado como uma cópia de volume ou um volume de snapshot. O volume será um dispositivo disponível para ser usado na criação de um novo datastore. Para obter mais informações sobre como montar volumes clonados e lidar com LUNs de snapshot, consulte a documentação da VMware sobre  https://techdocs.broadcom.com/["Montagem de uma cópia do datastore VMFS"^] e  https://techdocs.broadcom.com/["Gerenciando armazenamentos de dados VMFS duplicados"^].
